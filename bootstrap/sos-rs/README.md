...
| `adopt` | **Parity (hard-fail, stdout + tree-shape + gen-content + preservation)** — P077c4. Diffs stdout (`adopt.golden`, re-froze against a synthetic fake-kit + 4-collision BROWNFIELD target + `DOCTOR_BIN=/nonexistent/doctor` — the PREVIOUS golden had accidentally captured the CONNECTED `verify-setup`/`validate-map` path, same host-artifact class as pre-c3 `new.golden`), `adopt.tree.golden` (sorted path-shape, excludes `.git*`, INCLUDES `.sos-adopt-incoming/**`), `adopt.gen.golden` (content-hash, GENERATED-authored only), and an in-test **preservation-assert** (NOT a golden — universal non-clobber property: every seeded pre-existing file's sha256 unchanged before/after, every `.sos-adopt-incoming/<path>` byte-matches its kit source). Bug-for-bug: [2/4] calls into `map`'s scan logic AFTER kit assets are already copied into the target — AGENT_MAP.yaml maps those kit assets too (OA-02), fixed in P077c5. Non-clobber `added`/`conflicts` list order = raw traversal order (not sorted), same class as `sync`. |
