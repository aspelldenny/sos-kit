# PHIẾU P039: Doc drift + symmetry sweep
...
4. **No batched commits.** One commit per task (or one commit per logical group: Task 2a/b/c can be one commit "chore: replace hardcoded personal paths"). Use `git add -p` for surgical staging if needed. Branch = `phieu/P039-doc-drift-sweep`.
...
- [ ] `git diff --stat phieu/P039-doc-drift-sweep` shows ≤10 files changed (matches "Files cần sửa" table). If `docs/ticket/P004-...` Worker decided to fix, +1 OK.
- [ ] No file outside the "Files cần sửa" list modified (run `git diff --name-only main...phieu/P039-doc-drift-sweep` and audit).
...
### Docs Gate
- [ ] `CHANGELOG.md` — new entry at top: "P039: doc drift + symmetry sweep — refreshed CLAUDE.md tree (13 skills, all folders), removed 5 hardcoded personal paths, synced PHILOSOPHY/LAYERS/HANDOFF cross-doc inconsistencies, ported pre-commit Active sprint fallback from session-start-banner. Tầng 2 surgical, no behavior change except pre-commit fallback."
