...
- [ ] **[P076] Claude Code adapter parity — NEXT SESSION** — tái dựng wiring Claude từ cùng core; golden behavior trước/sau phải tương đương. Chưa mở phiếu/branch. Plan: `docs/plans/P076-claude-adapter-parity.md`.
...
- [ ] **[P084]** 🌟 **INPUT SENTINEL — gate cái AI ĂN VÀO, không chỉ cái AI ĐẺ RA** (Sếp nêu 2026-07-22, class mechanism mới). **Chẩn đoán:** mọi gate hiện có (`security-gate` INV, `docs-gate`, `trust-gate`, `no-code-on-default`, `lane-check`, `boundary-check`) đều canh **OUTPUT** LLM emit. **Không cái nào canh INPUT LLM ingest.** Doctrine ĐÃ gọi tên lỗ hổng này — `docs/PHILOSOPHY.md:107-117` §"garbage-in blind spot": *"the kit guards everything the LLM emits and nothing it ingests. A **future data sentinel** … would close it. Until then the reflex stays **manual**."* → **doctrine có, mechanism chưa.** **Sếp làm sắc hơn doc:** doctrine khung hẹp (DATA runtime, ca Soul Signature 2026-06-05 ciphertext/digest). Sếp mở rộng sang **repo = input cho AI agent**: docs bẩn, code bẩn, docs≠code = context Kiến trúc sư/Thợ đọc vào để suy luận → chưa gate. **Bằng chứng sống:** P083 (F2 `LAYERS.md` mang khung v1 "Claude Web Project" mâu thuẫn v2) — Architect đọc làm input → tự-kiến-trúc-sai; **không gate nào bắt, một người ngoài bắt.** **Mầm sẵn có:** `doctor validate-map` (path-check AGENT_MAP, bắt doc trỏ file không tồn tại) = input-sentinel thu nhỏ. `docs-gate` chỉ reactive (code đổi → ép update doc *của surface đó*), KHÔNG quét toàn repo tìm drift có sẵn → nên không bắt F2. **Cần (cheapest-80% §0.1):** một `sos gate input` / `sos hygiene` quét repo-integrity (docs↔code khớp · doc trỏ file thật · không residue chết) **TRƯỚC khi AI ăn context** — "code dọn bàn sạch trước, LLM ngồi viết sau" thành GATE thay vì reflex. **Host tự nhiên = Rust framework P077** (sống thành `sos` gate, agent-agnostic). **Liên quan:** section C "repo nhiễm độc" (adopt-time instance — input-sentinel là generalization chạy liên tục, không chỉ lúc adopt). **Trạng thái:** capture-only, build sau P077 khi có Rust gate host. Sếp: "đưa vào backlog trước, làm sau."
