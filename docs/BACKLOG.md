...
- [ ] **[P060–P063] HARVEST batch — downstream BUILD dogfood (claude-hooks + doc-rotate Rust port) 2026-06-09.** **VERDICT: sos-kit dogfood ĐẠT.** Workflow cõng trọn 1 port Rust nhiều phiếu (doc-rotate RP01→RP07b COMPLETE, 72/72 test, cargo install chạy, Python retired) + 1 partial (claude-hooks P001-P002). **Cú cứu thật (F04):** ARCHITECTURE ghi "byte-exact" nhưng code lưu **char offset** → Architect docs-only nuốt giả định sai vào phiếu RP02 → nếu lọt sẽ corrupt MỌI entry sau ký tự tiếng Việt đầu; Worker CHALLENGE + oracle-3-field BẮT pre-ship (v2.2 §2 HOLD trên PARTIAL oracle). Friction logs: `~/claude-hooks/docs/SOS_KIT_FEEDBACK.md` (F-001..F-003) + `~/doc-rotate/docs/WORKFLOW_FRICTION_LOG.md` (F01..F08 + pilot scoreboard).
  ...
  - [ ] **[P060]** 🔴 **`sos adopt` KHÔNG cài `docs/ORCHESTRATION.md` xuống downstream** (F01) — CLAUDE.md/banner trỏ spec file vắng mặt → orchestrator chạy "mù handbook" (phải tự suy state machine từ hook text). Fix: adopt copy ORCHESTRATION.md, HOẶC downstream docs trỏ `~/sos-kit/docs/ORCHESTRATION.md`. Adopt-gap, rẻ.
  - [ ] **[P061]** **Phiếu lifecycle không bootstrap khi adopt + convention track-or-ignore mơ hồ** (F-002 + F03). `phieu`/`.phieu-counter` không có sẵn → phiếu làm tay, hay bị untracked; `git mv active→done` FAIL "not under version control". Quyết DỨT: (a) phiếu = local artifact → `.gitignore phieu/`, HOẶC (b) audit trail → track + adopt seed `.phieu-counter` + `worker.md` ép `git add` phiếu. Hiện nhập nhằng dễ mất phiếu.
  - [ ] **[P062]** **DOCS GATE trigger thiếu hook PHASE-NUMBER** (F08). CLAUDE.md "Phase 5" stale 3 phiếu trong khi hook là `[8/8]` (drift âm thầm). Trigger table chỉ bắt "section add/remove", không bắt đổi-số. Fix: thêm hook-integrity test `grep -c 'blue "\['` so với doc-khai-báo, HOẶC trigger row "hook phase COUNT → CLAUDE.md/ARCHITECTURE". Rẻ.
