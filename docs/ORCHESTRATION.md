...
13. **Background subagent spawns (`run_in_background`).** Native Claude Code capability — NOT something the kit ships. Spawn a subagent with `run_in_background: true` (`Agent`/`Task` tool) and it runs async; the harness re-invokes the main session with a `task-notification` on completion. The orchestrator SHOULD use it so Chủ nhà isn't blocked on long, independent work (e.g. one approved Worker EXECUTE running while Chủ nhà does something else; a read-only specialist scan that takes minutes; independent phiếu on non-overlapping surfaces). **Discipline (the part the kit governs):** (a) **Marker exclusivity is load-bearing** — `architect-active`/`worker-active` are mutually-exclusive (Rule 6), so at most ONE guarded agent (architect OR worker) may be in flight at a time; two parallel Workers clash on `worker-active` + the same files. Read-only specialists (`advisory-watch`, `boundary-check`) carry no marker → several may run concurrently. (b) For a backgrounded Worker, keep `worker-active` set WHILE it runs and `rm -f` only on its **completion notification**, NOT right after the spawn call — removing it early closes the orchestrator-guard write-window mid-EXECUTE (Rule 12). (c) **APPROVAL_GATE still precedes EXECUTE** even when EXECUTE is backgrounded — background is hands-free, not gate-skip (Rule 4 unchanged). (d) **Never poll** — the harness auto-re-invokes on the notification; sleeping/looping to "check" wastes turns. Narrate in-flight background agents (Rule 2 — no silent state); a 1-line status table of running background work is good practice. Handbook mirror: `agents/orchestrator.md` "Background subagent spawns".
