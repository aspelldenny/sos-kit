...
**GATE_DELEGATION (first-class, IG-03 inv-gate 2026-06-11 — formalizes what Chủ nhà was doing by mouth):**
Chủ nhà may EXPLICITLY delegate APPROVAL_GATE to Quản đốc for a bounded scope (a sprint / a phiếu
class, e.g. "P002-P005 thuần kỹ thuật — em tự duyệt, báo cáo cuối"). While delegated, Quản đốc
self-approves ONLY when CHALLENGE is clean (every objection closed with an oracle, not prose).
Delegation NEVER covers: scope/vision change, security-surface decisions, irreversible actions
outside the repo — those still stop and ask. Delegation is per-grant, not sticky: a new sprint
resets to ask-mode unless re-granted. Log the grant in the phiếu Debate Log ("gate delegated by
Chủ nhà <date>, scope <X>") so the audit trail shows WHO approved. Related precedent:
[[feedback_skip_approval_gate]] (Tầng-2 drift fixes).
...
6. **Marker file hygiene (two markers, two guards).** `.sos-state/architect-active` gates `architect-guard.sh` (blocks Architect READING source); `.sos-state/worker-active` gates `orchestrator-guard.sh` (allows product-source Edit/Write only while set — see Rule 12). They are mutually exclusive phases. Orchestrator must, before spawning Architect: `mkdir -p .sos-state && touch .sos-state/architect-active && rm -f .sos-state/worker-active`; before spawning Worker: `mkdir -p .sos-state && touch .sos-state/worker-active && rm -f .sos-state/architect-active`; **after Worker returns: `rm -f .sos-state/worker-active`** (close the write window); **after Architect returns: `rm -f .sos-state/architect-active`** (stale architect marker turns architect-guard against the main session itself — its Write-allowlist then blocks Quản đốc's legit `.md` edits; hit ×2 in inv-gate Sprint 1, IG-01). Never leave stale markers. (Markers live outside `.claude/` so YOLO mode doesn't prompt — `.claude/` is gated even with `--dangerously-skip-permissions`.) **No orchestrator git-op while worker-active (IG-12 inv-gate Sprint 3):** the tree belongs to the Worker during EXECUTE — Quản đốc must not `git commit`/`checkout`/`add`/`stash` the shared tree while `worker-active` is set (the Worker commits, step 8). Real race: orchestrator committed P009 evidence while a P010 Worker had a branch checked out in the same tree → staged into the Worker's WIP; fail-closed build-guard caught it. For DELIBERATE parallel-phiếu fan-out, spawn each Worker with the Agent tool's `isolation: "worktree"` (own git tree) — the sequential default needs no worktree (shared tree safe one-at-a-time).
