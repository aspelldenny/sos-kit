...
**Rust cutover (P077e):** the 6 heavy `sos` subcommands — `new`/`adopt`/`sync`/`map`/`install`/`tools` — now dispatch (`exec`) to the canonical Rust `sos` binary built from `bootstrap/sos-rs/`; `bin/sos.sh` fails LOUD (no silent Bash fallback) if it can't resolve one. For a sos-kit dev checkout: `cd bootstrap/sos-rs && cargo build --bin sos` (or set `SOS_RUST_BIN=/path/to/sos`). Prebuilt-binary bootstrap for end users (no local Rust toolchain) is deferred to **P081**; today `install.sh` stays zero-touch (unchanged). The 7 guidance subcommands (`init`/`blueprint`/`contract`/`apply`/`recipe`/`launch`/`status`) are unaffected — still plain Bash.
