...
> **Status:** P077d1 SHIPPED (2026-07-22) — `Adapter` trait (5 method) + `ManagedManifest` schema (6 field) carved in `sos-core`, `sos-adapter-claude` implements trait (stub bodies, zero fs mutation). **P077d2 SHIPPED (2026-07-22)** — install ENGINE live in `sos-install::engine`, `sos install --runtime <r> [--dry-run]` wired in `sos-cli`, 5 correctness fixtures (MockAdapter) green + 20× parallel 0-flaky. Narrow d1 amendment: `ManagedOperation` gained `content: String` (Worker CHALLENGE O1.1, Architect ACCEPT Alt A) — `Adapter` 5-method + `ManagedManifest` 6-field unchanged. **P077d3 SHIPPED (2026-07-22)** — `tool-manifest.toml` (kit root) pins 10 sister tools (6 required/4 optional), `sos_install::tools::{check_tools, gate_required}` core, `sos tools status` + step-5 `resolve_tools()` fill, both surfaces sharing one gate. 6 fixtures green + ×20 parallel 0-flaky. Live smoke reproduces OA-07 exactly. **P077d CLOSED (d1+d2+d3) → next P077e cutover.**
...
| **P077d2** | ✅ **SHIPPED** — **Install engine**: transaction plan / `--dry-run` / non-clobber / rollback record / apply trên kit+adapter assets (`PORTABILITY_ARCHITECTURE.md:107-115` 7-step, TRỪ step 5 tool-resolve = d3 seam). `sos install` NEW, chạy SONG SONG `install.sh` (install.sh + bin/sos.sh KHÔNG đổi). Consume d1's manifest schema + adapter trait (narrow amendment: `ManagedOperation.content` added). | **Install correctness fixtures** (additive / non-clobber / rollback / idempotence / dry-run), hard-fail — 6/6 PASS, ×20 parallel 0-flaky. KHÔNG parity-vs-Bash. | Guarded | ✅ (alongside install.sh) | ❌ (additive) — ⚠️ optional eyeball CHỈ nếu đổi install.sh default | P077d1 |
| **P077d3** | ✅ **SHIPPED** — **tool-manifest.toml (OA-07)**: pin version+platform-asset+checksum cho 10 external tools (6 required/4 optional, khớp install.sh) + `sos tools status` (expected vs installed drift) + fail-clear khi required tool thiếu/version cũ (KHÔNG `sos doctor` subcommand — surface qua `tools status` exit + step-5 gate). Fill d2's step-5 tool-resolve seam. Checksum = honest TODO placeholder (E2, real fill = P081). | **Manifest-pin-verify + status-drift + doctor/step-5 fail-clear fixtures — 6/6 PASS, ×20 parallel 0-flaky.** Synthetic fake-PATH stub tools, zero dependency on real installed binaries. | Guarded | ✅ | ❌ (additive) | P077d2 |
