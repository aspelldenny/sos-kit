...
- **Q-D5 — verify-WIRING cơ chế nào?** ✅ **[RESOLVED R2 framing — 3 nguồn hội tụ]:** `doctor verify-setup` = subcmd thứ 5, **TĨNH/SOUND** (đọc file/grep/check tồn tại — KHÔNG spawn; file-checked: 4 subcmd doctor hiện tại đều static 0-spawn → canary KHÔNG nhét vào đây kẻo bẩn binary sound). Kiểm chuỗi nối (sentinel khớp / command tồn tại / INVARIANTS tồn tại nếu enabled / auto-spawn / merge-hook / inject-path). Reference = **tarot-WIRING** (wiring tĩnh kiểm được, tarot là mẫu đã-chạy). Nhãn cứng: **kiểm NỐI, KHÔNG kiểm TỐT.** Chạy bootstrap + pre-commit. Un-defer từ spec. → quality KHÔNG ở đây, xem **Q-D7**. — ✅ **[BUILT+RAN Vòng 12: discrimination PASS (doc-rotate DORMANT / tarot CONNECTED); design STANDS. Chi tiết §5 Vòng 12.]**
...
- **Vòng 12 (2026-05-29, session sau clear) — ✅ Q-D5 BUILT + RAN: verify-setup ĐỨNG (thứ chạy-thật #4 của arc).**
  - **Build:** `doctor verify-setup` = subcmd thứ 5, STATIC/SOUND (đọc file + regex + string-compare, **0 spawn**, cùng họ 4 subcmd cũ). Scope HẸP = chuỗi wiring vai Giám sát. **5 joint, MỌI joint derive từ tarot-wiring THẬT (không bịa):** J1 sentinel-contract / J2 rubric-source / J4 invariants-file / J5 merge-gate (registered settings.json) / J6 verdict-contract. cargo: **38 test xanh** (5 fixture verify-setup mới + regression mcp tools 4→5). Code: `~/doctor/src/cli/verify_setup.rs` + main/mcp wired.
  - **Checklist KHÔNG tự-nghĩ — Pin #1 CỨU cái build:** workflow 3-agent (oracle tarot ground-truth + oracle doc-rotate expected-output + adversarial blind-spot critic). Critic bắt **3/5 joint GỐC sẽ báo NHẦM tarot DORMANT**: J2 "command phải inject" (tarot inline rubric, KHÔNG inject), J3 "auto-spawn" (tarot dùng block-on-absence merge-gate, KHÔNG auto-spawn), J5 "git-hook" (tarot wire qua `.claude/settings.json` PreToolUse). Nếu viết từ checklist-tự-nghĩ → tool MÙ (báo CẢ HAI repo DORMANT) → fail discrimination. **Đúng nỗi sợ §2: "cùng cái đầu mã hóa failure-mode-doc-rotate thành success-requirement."** Critic câu nói trúng: *"retro 4-fracture mô tả doc-rotate CHẾT thế nào, KHÔNG phải mối-nối-tối-thiểu khiến tarot SỐNG."*
  - **DISCRIMINATION TEST (Pin #2 — canary cho CHÍNH verify-setup) PASSED:**
    - doc-rotate → **DORMANT (exit 1)**: `emit 'security-review-start' != grep 'SECURITY_REVIEW_START' (case/separator); docs/security/INVARIANTS.md missing` = ĐÚNG cái báo động §2 "đáng-lẽ-cứu-anh lúc bootstrap".
    - tarot → **CONNECTED (exit 0)**: 5/5 joint WIRED.
    - sos-kit (data point #3) → DORMANT chỉ vì J4 (ships `templates/INVARIANTS-template.md`, không file thật). **J1 lowercase MATCH ✓** → chứng tool so-string THẬT, KHÔNG ghét-lowercase: doc-rotate lowercase-emit-vs-UPPER-grep MISMATCH nhưng sos-kit lowercase-cả-2-đầu MATCH.
    - **Kết luận: tool KHÔNG mù (tarot CONNECTED) + KHÔNG vẹt (doc-rotate DORMANT). Hai repo đáp-án-trái-ngược tách đúng → Q-D5 design ĐỨNG, RUN-PROVEN không còn paper.**
  - **Residual / hardening (GHI, KHÔNG làm tối nay):**
    - 2 joint critic xác nhận static-sound nhưng HOÃN có lý do file-grounded (KHÔNG defer-vì-ngại): **J7** surface-pattern agreement (tarot blocking-command có `SECURITY_SURFACE_PATTERN` vs doc-rotate advisory-command không có → khác kiến trúc); **J9** spawn-name==agent-name (tarot còn ref CŨ `@agent-security-reviewer` ở security-review.md:83 → naive check báo nhầm tarot). Cần arch-aware extraction.
    - **J4 conditional:** sos-kit/kit ships template → DORMANT-noise. Refine theo critic mitigation: "require khi security-enabled / rubric-source là file (mode a); inline (mode b) → optional." Hardening, không gấp.
    - ⚠️ **NHÃN brittle-ở-rìa (Claude push, em nhận):** Q-D5 proven **phân-biệt-được trên 3 repo wiring-pattern ĐÃ BIẾT** (tarot/doc-rotate/sos-kit), **CHƯA proven trên repo pattern LẠ** (cách-sống thứ-4 mà cả tarot lẫn doc-rotate không trưng). Bằng chứng brittle: suýt báo-nhầm-tarot vì ref cũ `@agent-security-reviewer` (J9). = cùng họ M1 cả phiên (đúng-trên-data-đã-thấy, chưa chắc data-mới). **Đừng để "discrimination PASS" nở thành "tool đúng mọi nơi" — nó đúng 3 nơi, brittle ở rìa.**
  - **Pin #4 GIỮ:** phiên này CHỈ VALIDATE TOOL. **KHÔNG sửa doc-rotate sentinel / thêm INVARIANTS.md trong cùng nhịp** (= nhịp khác). Một phép đo một lần — giữ cái sạch của bài test.
  - **Trạng thái:** Q-D5 🔨→✅ **built+ran+stands**. Arc giờ có **2/6 verdict chạy-thật** (Q-D7 canary partial @Vòng10 + Q-D5 verify-setup @Vòng12). Còn: Wave A (Q-D4 slot + Q-D3 annotate + Q-D6 note) + Q-D2 mầm-bệnh auto-Guarded → rồi Wave D. **STOP trước WORKFLOW_V2.3.md/golden — chờ Sếp final-approve.**

- **Vòng 13 (2026-05-29) — 🔴 SPAWN-DISCRETION BUG (media pilot, cross-pilot finding — đào DƯỚI §2):** Pilot media-rating chạy phiếu INV-LOCAL-001 (security/privacy gate). **Bằng chứng cơ học** (đếm `subagents/agent-*.jsonl` của 2 phiên):
  - doc-rotate (hôm qua, delegation "em là quản đốc, approval gate giao em, chạy end-to-end dự án"): **23 spawn** (9 architect + 14 worker), nhịp DRAFT→CHALLENGE→RESPOND→EXECUTE sạch P001–P004.
  - media INV-LOCAL (hôm nay, delegation GẦN HỆT "quản đốc, approval gate toàn quyền, làm xong báo cáo"): **0 spawn** — main session tự đọc code, tự viết phiếu, tự CHALLENGE, tự approve, tự EXECUTE, tự PR; boundary-check tụt thành "đề xuất optional sau".
  - **Khác biệt DUY NHẤT giữa 2 prompt:** "chạy DỰ ÁN (lớn)" vs "làm MỘT phiếu nhỏ (~3 dòng)". → **prompt KHÔNG phải thủ phạm** (Sếp kích y hệt hôm qua). Đừng gọi đây là "prompt bug của user".
  - **GỐC:** spawn-vai đang **DISCRETIONARY**; model suy độ-sâu-orchestration theo **kích-thước-cảm-nhận** (LOC nhỏ→solo) thay vì **surface/Tầng** (security→AUTO Tầng-1, Rule #8). LOC-nhỏ bị lẫn Tầng-thấp → security-Tầng-1 chạy solo, người-kiểm == người-làm == tự khen tự duyệt (phá đúng cái workflow sinh ra để chống).
  - **ĐÀO DƯỚI §2 (NÚT TRUNG TÂM):** verify-setup check **wiring CONNECTED**, NHƯNG CONNECTED ≠ **vai-được-SPAWN-lúc-runtime**. Media wiring đủ → verify-setup BÁO CONNECTED → mà 0-spawn. Bug nằm **DƯỚI tầng static verify-setup**: trục mới **verify-vai-CHẠY-RUNTIME** — không static check nào bắt được, phải cơ chế runtime.
  - **NGUYÊN LÝ (Sếp — sắc nhất phiên):** trigger KHÔNG được dựa trí nhớ của **BẤT KỲ AI NÀO**, kể cả con AI: user không gõ `/build` (nói tiếng Việt; lệnh-phải-nhớ = **chết yểu**) + model không tự-nhớ-gọi (= coin-flip vừa solo). → "đừng bắt LLM nhớ" MỞ RỘNG: đừng bắt **founder** nhớ + đừng bắt **model tự-nhớ-gọi**. Cơ chế ép phải nổ trên **HÀNH ĐỘNG VẬT LÝ** (sửa code / tạo PR / nói "xong"), độc lập trí nhớ.
  - **Prose-rule = VÔ DỤNG (proven):** contract ĐÃ cấm solo (`orchestrator.md:17` + anti-pattern #1) → vẫn bị lờ. Thêm prose nữa = lại bắt-nhớ. Fix BẮT BUỘC là MECHANISM, không phải câu-chữ.
  - **Fix direction (CORRECTED Vòng 14 — cross-check bắt điểm mù; tách SOUND khỏi PARTIAL):** Bản đầu (em + ChatGPT) định dùng HOOK để *đoán* "việc này chạm security → ép Tầng-1" = **phân-loại-mức-nguy = PARTIAL** → grep/keyword/path sẽ TRƯỢT ca security-không-lộ-tên = **cài lại mầm bệnh Q-D2 vào runtime**, gói trong chữ "hook hành-động-vật-lý" nghe-cơ-học-nên-nghe-đúng. Bản đúng:
    1. **Tầng = phán đoán NGƯỜI, KHÔNG model-đoán-từ-LOC.** Nguồn sound (giải luôn "Sếp đâu viết phiếu"): **INV-LOCAL/INVARIANTS membership** — phiếu implement INV-LOCAL-X → tự Tầng-1 (vì Sếp ĐÃ tuyên invariant đó security trong INVARIANTS.md lúc bình tĩnh). Non-INV: architect đề xuất, Sếp chốt ở approval gate. Membership = sound, KHÔNG grep-diff-đoán-security.
    2. **Hook chỉ ép cái SOUND (kiểm tồn-tại, KHÔNG phán nội-dung):** (a) phiếu thiếu field Tầng → không chạy; (b) Tầng-1 → vai bắt-buộc đã spawn thật chưa = **state-check verify-vai-CHẠY** (đếm spawn/marker — salvage từ ChatGPT, = proof 23-vs-0); (c) Tầng-1 PR → có boundary-check verdict chưa (`block-unsafe-merge` đã làm nửa merge). **KHÔNG hook nào đoán "việc này có phải security".**
    3. `/build` = convenience agent-internal (GUIDES), KHÔNG phải lớp enforce, KHÔNG phải lệnh user gõ.
    - **Nguyên lý:** máy ép **SỰ-CÓ-MẶT của phán đoán người** (sound); máy **KHÔNG thay người PHÁN mức-nguy** (partial). → fix này **ĐÓNG LUÔN Q-D2 mầm-bệnh** (Tầng=judgment-người-gán, không keyword-auto). Incident hôm nay = bằng chứng sống Q-D2 (model dùng heuristic LOC xếp Tầng → trượt).
  - **Incident sống + PROOF:** media **PR #10** = sản phẩm solo-run này → chạy `/security-review 10` (post-hoc) = **NEEDS_REVIEW**: Giám sát bắt **3 gap solo-run bỏ** — (a) header-counts policy, (b) **ghost-existence-leak qua `taste_dna`** (302 vs 404), (c) **`social.py` followers/following chưa khoá private** (b,c = leak THẬT). → **demo sống: vai bị-skip khi CHẠY THẬT bắt lỗ tác-giả-không-thấy** = giá trị role-separation, không phải lý thuyết. Verdict lưu `media/docs/security/last-review.md`. (INV-LOCAL-001 còn chưa đủ: home `/`+feed+media-detail rò cho khách ẩn danh — sweep ngoài diff PR.)
...
- **WAVE PLAN hạ (Sếp R9 — chia sóng, KHÔNG hạ một lượt):**
  ...
  - **Wave B:** ✅ **DONE (Vòng 12)** — BUILT `doctor verify-setup` (5 joint, static/sound, 38 test xanh) + **chạy thật**: doc-rotate→DORMANT(exit1), tarot→CONNECTED(exit0), sos-kit→DORMANT(J4-only). Discrimination PASS → **Q-D5 design STANDS**. Residual hoãn-có-lý-do: J7/J9 (arch-aware) + J4-conditional (kit-vs-project).
  - **Wave C:** ✅ **PARTIAL DONE (Vòng 10)** — tarot v166 known-class canary (4/4 lỗi cấy caught, real-gate, production-risk 🔴→🟡). **Remaining:** end-to-end `/security-review` harness HOẶC adversarial/subtle canary TRƯỚC khi doctrine overclaim. ⚠️ Đừng đọc nhầm Q-D7 = "chưa có evidence nào" (Codex catch) — proxy canary done, full harness pending.
  - **Wave D:** chỉ SAU khi Q-D5/Q-D7 có evidence thật **+ Q-D2 mầm-bệnh-auto-Guarded đã xử** → mới viết `WORKFLOW_V2.3.md`/golden.
  - 🔴 **advisory→block (2-class chí tử: payment-webhook spoof + cross-user leak) — residual consequence-production thật nhất còn mở** (Vòng 11 reweight; Claude+Codex đều nâng). **NHƯNG có PRECONDITION (em thêm — KHÔNG rubber-stamp "consequence nên làm trước"):** advisory→block là HARD gate; nếu cắm lên detector mới canary **textbook known-class** (Q-D7 partial, CHƯA adversarial) → đẻ **FALSE SECURITY** (block-forever false-positive → bị disable → dormant lại; HOẶC pass subtle attack mà cho cảm-giác-an-toàn "gate cứng rồi khỏi đọc comment" = TỆ HƠN advisory vì advisory ít nhất giữ người trong vòng). → **advisory→block GATED ON Q-D7-full (adversarial/end-to-end)**, không pull trước precondition. Chuỗi đúng: Q-D7-full canary → rồi advisory→block (Wave-D-tier).
