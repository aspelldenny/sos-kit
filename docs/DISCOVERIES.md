...
## [P038] — 2026-05-05 — Doc drift + symmetry sweep (10 surgical edits, Tầng 2)
...
### Task 9 occurrence count
- Architect predicted 3 occurrences at lines 17, 81, 190. Actual: 5 occurrences at lines 17, 81, 155, 190, 200. Two additional: line 155 in Step 3 Phiéu workflow init prose, line 200 in Integration with Other Skills section. All 5 replaced. Tầng 2 self-decide (same task scope, no new file touches).
...
- **P036 retrospective signal:** Skip is safe when: (a) all anchor types are correctness-of-fact, not architectural-assumption; (b) scope is clearly bounded string replacements; (c) Architect marked uncertain anchors with `[needs Worker verify]` instead of false `[verified]`. P038 confirms P037's pattern.

### Total time
- Architect estimated ~50 minutes. Actual execution: ~35-40 minutes (10 tasks + validation greps + CHANGELOG + Discovery). Tầng 2 surgical phiếu well-scoped.
...
- `CHANGELOG.md`: v2.1.7 entry added.
