...
- ✅ **Freeze (proven-cắn):** agent `.md` files (spine A1), **generic SOS skills** (the 13: plan/verify/review/qa/ship/retro/init/idea/insight/route/decide/forge/apply → `.claude/skills/`), scripts (`block-env-edit`, `block-unsafe-merge` **with the FIXED sentinel**, `architect-guard`, `session-start-banner`), `phieu/` workflow, `templates/`, `.claude/commands/`, **`.mcp.json` wiring OUR doctor gate** (PATH-rel `doctor`, never a hardcoded `~/.cargo` path), dir structure.
- 🧭 **Freeze-test = ATTRIBUTE filter, NOT item-type list (2026-05-29 refine — closes the skills/mcp gap).** For ANY artifact ask 2 questions: (1) same-across-every-repo or different-per-repo? (2) in-our-control or external-dependency? → same + in-control = **freeze (Cat-A)**; different-per-repo = **Cat-C (survey/declare, do NOT copy)**; external-dependency = outside both tiers, careful with credentials. Consequences:
  - **Skills split:** the 13 generic SOS skills = same-everywhere + in-control = freeze (copy). Domain skills (a repo's own `phase-gate`/`status`, lumi-lighting, tarot-card) = Cat-C — adopt KEEPS them, never copies them cross-repo. ("Does skill X freeze?" is the wrong question — filter each.)
  - **MCP split by OWNERSHIP:** tools WE code (`doctor`/`ship`/`guard`/`vps`/`docs-gate`) = our-own, in-control, no-credential = freeze the wiring (PATH-rel). External MCP (context7/supabase/...) = Cat-C — the repo declares its own; the kit does NOT ship them, so the credential-risk evaporates. *Spirit: LLM reasons; our own tools are the deterministic hands.*
  - **Same root as the map-lie gap:** adopt must eventually SURVEY/declare the per-repo (which domain-skill, which external-MCP), not leave-blank (skill/mcp) or copy-wrong (the tarot AGENT_MAP lie). Survey step = curate (not built). For now adopt adds the generic skills + wires doctor; the per-repo it leaves to the human.
...
### 8.1 `sos map` — scan-grounded AGENT_MAP (fix for the tarot-content-copy gap, 2026-05-29)
The media-rating adopt exposed a real adopt BUG: it copied `configs/AGENT_MAP.example.yaml` CONTENT (tarot surfaces `parser`/`api_boundary`/`character_voice` + P305 blast) into the new repo + a TODO line = a **"map nói dối"** (Architect would route by a FALSE map — worse than no map). This violates the retro's own §0/§1 rule (*tarot = wiring-PATTERN, NOT repo CONTENT*). Root cause: adopt treated AGENT_MAP (Category C — must-survey content) like Category A (copy template). `doctor validate-map` would even have caught it (tarot paths don't exist) — a second gap: adopt didn't run validate-map.

**Fix = `sos map [dir]`** — SCANS the repo → draft AGENT_MAP with REAL surfaces. SOUND half (auto): `routes_handlers / models_schema / services_logic / migrations / frontend / config_runtime` grouped from real dirs; edit-paths = real files. PARTIAL half (human): `load_bearing` defaults `true` (safe — over-read beats miss-blast; comment flags NEEDS_JUDGMENT) + `blast` = TODO. Same sound/partial split as `docs-gate init` (scan = framework, human = judgment). Coarse by design — semantic clustering (`routes_handlers` → auth / ratings / privacy) is the human's refine.

Wiring: `sos adopt` now calls `sos map` (scan) instead of copying the example; `sos new` emits a `draft_unmapped` stub (greenfield has nothing to scan yet). Acceptance PASSED: scanned map = real paths + 0 tarot leak + `validate-map` PASS; stub `validate-map` PASS; fresh-spawn pre-commit green. **Curate gap CLOSED.**

### 8.2 Comprehensive brownfield adopt E2E (clean clone of media-rating, 2026-05-29)
Ran `sos adopt` on a CLEAN git clone of media-rating-app (committed state only — no dirty / prior-adopt contamination) against a 7-point checklist. Surfaced + fixed two real adopt gaps, then green:
- **Born-ready brownfield (fix):** adopt now GENERATES Cat-C doc skeletons (CHANGELOG / ARCHITECTURE / BACKLOG) when ABSENT (additive), so the adopted repo passes its OWN pre-commit (docs-gate ✅ + BACKLOG ✅). Was failing because adopt reported-but-didn't-generate them (greenfield `sos new` did, brownfield didn't — an inconsistency). CLAUDE.md stays report-only (product identity, never auto-generated over a brownfield).
- **validate-map self-check (fix — the §8.1 follow-up, done):** adopt now runs `doctor validate-map` post-scan — automatically catches the map-lie class (paths must exist), the gate that would have caught the original tarot-copy.
- **RESULT — adopt's responsibility = FULLY GREEN:** non-clobber (0 tracked files modified; CLAUDE.md byte-identical) · verify-setup CONNECTED · map scan + 0 tarot + validate-map PASS · INVARIANTS.md present · docs-gate ✅ + BACKLOG ✅ · 0 conflicts staged.
- **The one remaining pre-commit ❌ = the security-gate WORKING, NOT an adopt bug:** INV-010 flagged inline `postgresql://…:password@` db-creds in media's `.github/workflows/ci.yml` + `docker-compose.yml` (test/CI creds). The gate adopt installed immediately caught real content = teeth on the real repo. Media's own workflow triages (allowlist test-creds vs real fix) — content, not platform lane. (Possible gate-tuning curate: should INV-010 allowlist known test-db-cred patterns?)
