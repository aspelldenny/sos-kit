...
Wiring: `sos adopt` now calls `sos map` (scan) instead of copying the example; `sos new` emits a `draft_unmapped` stub (greenfield has nothing to scan yet). Acceptance PASSED: scanned map = real paths + 0 tarot leak + `validate-map` PASS; stub `validate-map` PASS; fresh-spawn pre-commit green. **Curate gap CLOSED.** (Follow-up: adopt could run `validate-map` post-scan as a self-check.)
